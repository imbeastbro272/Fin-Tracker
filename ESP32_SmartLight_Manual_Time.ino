 ...
 * - MANUAL TIME INPUT via serial monitor (hh:mm:ss format)
 *
 * Hardware:
 * - ESP32 DevKit
...
float previousPotAdjustment = 0.0;
float currentBrightness = 0.0;
float serialAdjustment = 0.0;
bool useSerialAdjustment = false;
...
void setup() {
    ...
    Serial.println();
    printSeparator('=', 80);
    Serial.println("ESP32 SMART LIGHT WITH EMBEDDED ML - MANUAL TIME MODE");
    printSeparator('=', 80);
    ...
    pwmWrite(LED_PWM_PIN, LED_CHANNEL, 0);
    ...
    Serial.println("  - Time mode: MANUAL (default 12:00:00)");
    Serial.println();
    printHelp();
    printSeparator('=', 80);
    Serial.println();
...
void updateManualTime() {
    ...
    if (elapsed >= 1000) {
        ...
        manualSecond += secondsPassed;
        
        while (manualSecond >= 60) {
            manualSecond -= 60;
            manualMinute++;
        }
        
        while (manualMinute >= 60) {
            manualMinute -= 60;
            manualHour++;
        }
...
void handleSerialInput() {
    ...
    if (input.indexOf(':') >= 0) {
        ...
        if (firstColon > 0 && secondColon > firstColon) {
            ...
            if (h >= 0 && h < 24 && m >= 0 && m < 60 && s >= 0 && s < 60) {
                ...
                Serial.println("ERROR: Invalid time. Use 00-23 for hours, 00-59 for minutes/seconds");
                return;
            }
        }
    }
    ...
    else if (inputLower == "help" || inputLower == "?") {
        printHelp();
    }
    else if (inputLower == "reset") {
        serialAdjustment = 0.0;
        useSerialAdjustment = true;
...
void printHelp() {
    ...
    Serial.println("  hh:mm:ss : Set time (e.g., 14:30:00 for 2:30 PM)");
    ...
    Serial.println("  Type '09:30:00' to set time to 9:30 AM");
    Serial.println("  Type '21:15:30' to set time to 9:15:30 PM");
...
float readLux() {
    ...
    const float LDR_A = 32768000.0f;
    const float LDR_B = -1.4f;
...
float readPotAdjustment() {
    ...
    float adjustment = ((float)raw / 4095.0f) * 100.0f - 50.0f;
    return adjustment;
}
